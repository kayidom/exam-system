...
class Exams extends DBConnection {
    ...
    public function insertExam($data, $filename) {
        $row = $this->db->prepare("INSERT INTO `exam_output`(`ModCode`, `exam_type`, `exam_date`, `start_time`, `end_time`, `number_of_questions`, `file_document`) 
                                VALUES (:module_code, :exam_type, :exam_date, :start_time, :end_time, :num_of_questions, :filename) ");
        $results = $row->execute([
    ...
    public function getStudentComingExams($studentNumber) {
        $row = $this->db->prepare("SELECT * FROM `admission` 
                                    WHERE `StudentNumber` = ? AND `ModCode` 
                                    IN (SELECT `exam_output`.`ModCode` FROM `exam_output`) ");
    ...
    public function getExamInfo($moduleCode) {
        $row = $this->db->prepare("SELECT * FROM `exam_output` WHERE `ModCode` = ? ");
    ...
    /*public function getAllExamRecords() { //WHERE EXTRACT(YEAR_MONTH, exam_date) = DATE_SUB(EXTRACT(YEAR_MONTH, CURRENT_DATE(), INTERVAL 3 MONTH) AND
        $row = $this->db->prepare("SELECT * FROM `examinations` WHERE exam_type = 'Document-Upload' ORDER BY id DESC");
        $row->execute();
        if($row->rowCount() > 0) {
            return $row->fetchAll(PDO::FETCH_OBJ);
        }
